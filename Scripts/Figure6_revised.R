...
max_y_value <- max(Filtered_data_5$median_wormlength_um, na.rm = TRUE)
...
Filtered_data_5 <- Filtered_data_4 %>% 
                   tidyr::separate(concentration_um,into=c("type","num"),remove = F)
...
p1 <- ggplot(data = Filtered_data_5, mapping = aes(x = type, y = median_wormlength_um)) +
...
ggsave("/vast/eande106/projects/Nikita/Filtration/Filtrationmanuscript/Plots/Figure6_revised.tiff", plot = p1, width = 5.2, height = 5.2, unit = "in", dpi = 600)


##Statistical tests
##Wilcoxon (Rank-sum (Mann-Whitney test) test to compare between bleach and filtration:
wilcox_results_1 <- wilcox.test(median_wormlength_um ~ type, data = Filtered_data_5)
print(wilcox_results_1)
#p = 0.00252

# Perform a non paramatric one-way ANOVA (Kruskall-Wallis) to assess variance explained by replicates
kruskal_test_result_replicate <-kruskal.test(median_wormlength_um ~ Replicates, data = Filtered_data_5)
print (kruskal_test_result_replicate)
##P value < 2e-16 which is statistically significant. This tells us that there is a large difference in the variance
#between replicates compared to within replicates The "replicates" factor is explaining a significant portion of
#the variance in the worm length data
