...
p1_c1 <- ggplot(data = poo_1 %>% dplyr::filter(class=="R"), mapping = aes(x = Method, y = median_wormlength_um_delta)) +
...
p1_c2 <- ggplot(data = poo_1 %>% dplyr::filter(class=="S"), mapping = aes(x = Method, y = median_wormlength_um_delta)) +
  # Jitter for individual data points
  geom_jitter(
    shape = 16,
    width = 0.15,
...
###Plot drug (panel 1)
p2_c1 <- ggplot(data = poo_2 %>% dplyr::filter(class=="R"), mapping = aes(x = Method, y = median_wormlength_um_delta)) +
  # Jitter for individual data points
  geom_jitter(
    shape = 16,
    width = 0.15,
...
p2_c1
#plot drug (panel 2)
p2_c2 <- ggplot(data = poo_2 %>% dplyr::filter(class=="S"), mapping = aes(x = Method, y = median_wormlength_um_delta)) +
  # Jitter for individual data points
  geom_jitter(
    shape = 16,
    width = 0.15,
  ...
  # Control the number of Y-axis ticks (always 3)
  scale_y_continuous(breaks=c(-150,-225,-300)) +
  ...
  coord_cartesian(ylim = c(-325,-140)) 
...
ggsave("/vast/eande106/projects/Nikita/Filtration/Filtrationmanuscript/Plots/Figure7_revised.tiff", plot = combined_plot_1, width = 5.2, height = 5.2, unit = "in", dpi = 600)
